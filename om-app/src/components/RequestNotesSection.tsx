...
import {
  getRequestNotes,
  addRequestNote,
  softDeleteRequestNote,
  type OmRequestNote,
} from '../services/requestNoteService'
import { colors } from '../theme'
import { ConfirmModal } from './ConfirmModal'
import { EditableField } from './EditableField'

// Sourced from the dom_om_note_type domain on the notes service.
// TODO: when domainService supports loading domains from related-table
// fields, pull this dynamically instead of hard-coding.
...
type Props = {
  /** Parent request's GlobalID — drives which notes get loaded. */
  requestGlobalId: string | null
  defaultOpen?: boolean
}

export function RequestNotesSection({ requestGlobalId, defaultOpen = false }: Props) {
  const [open, setOpen] = useState(defaultOpen)
  const [notes, setNotes] = useState<OmRequestNote[]>([])
  const [loading, setLoading] = useState(false)
  const [loadError, setLoadError] = useState<string | null>(null)

  // ---- Add-note form state ----
  const [showAddForm, setShowAddForm] = useState(false)
  const [draftType, setDraftType] = useState<string | null>(null)   // no default per design call
  ...
  const [deleteTarget, setDeleteTarget] = useState<OmRequestNote | null>(null)
  const [isDeleting, setIsDeleting] = useState(false)

  // Load notes when section opens or the parent request changes
  useEffect(() => {
    if (!requestGlobalId || !open) return
    let cancelled = false
    setLoading(true)
    setLoadError(null)
    getRequestNotes(requestGlobalId)
  ...
  const canSave =
    !!draftType && draftText.trim().length > 0 && !isSaving && !!requestGlobalId

  async function handleSaveNote() {
    if (!requestGlobalId || !canSave) return
    setIsSaving(true)
    setSaveError(null)
    try {
      ...
      // Refresh the list — easier than trying to inject the new row locally
      // (we don't have its GlobalID yet from the add response).
      const fresh = await getRequestNotes(requestGlobalId)
      setNotes(fresh)
      // Reset form
      setDraftType(null)
      setDraftText('')
  ...
  async function handleConfirmDelete() {
    ...
    setIsDeleting(true)
    try {
      await softDeleteRequestNote(deleteTarget.objectId)
          ...
          {!showAddForm ? (
            ...
            <div style={{
              ...
              <EditableField type="select" label="Type"
                ...
                onChange={(val: string | null) => setDraftType(val)} />
              <EditableField type="date" label="Note Date"
                value={draftDate}
                onChange={(val: number | null) => setDraftDate(val ?? Date.now())} />
              <EditableField type="textarea" label="Note" wide
                value={draftText}
                maxLength={4000} rows={3}
                onChange={(val: string | null) => setDraftText(val ?? '')} />
              {saveError && (
...
function NoteCard({ note, onDelete }: { note: OmRequestNote; onDelete: () => void }) {
  return (
    <li style={{
