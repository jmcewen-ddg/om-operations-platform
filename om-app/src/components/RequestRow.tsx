...
type Props = {
  request: OmRequest
  /** When false, the row renders without a checkbox. Use for statuses
      that aren't eligible for work-order assignment (anything other
      than 'Ready for Work Order'). */
  selectable: boolean
  isSelected: boolean
  ...
  return (
    <li
      style={{
        ...
        background: selectable && isSelected ? `${colors.blue}11` : colors.white,
        border: `1px solid ${selectable && isSelected ? colors.blue : colors.lightGray}`,
        borderLeft: `4px solid ${selectable && isSelected ? colors.blue : colors.lightGray}`,
        borderRadius: 4,
